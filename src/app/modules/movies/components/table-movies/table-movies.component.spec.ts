...
describe('TableMoviesComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [TableMoviesComponent, BadgeComponent]
    })
    .compileComponents();
  ...
  it('debería mostrar los datos de la película en cada columna', () => {
    ...
    rows.forEach((row, index) => {
      ...
      expect(columns[3].nativeElement.textContent.trim()).toBe(String(movie.year));
  ...
  it('debería mapear cada género a su BadgeType correcto', () => {
    ...
    expect(component.genreMap['Ciencia Ficción']).toBe('info');
    expect(component.genreMap['Acción']).toBe('warning');
    expect(component.genreMap['Comedia']).toBe('success');
